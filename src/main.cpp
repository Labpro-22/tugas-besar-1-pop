...
static std::map<int, ComputerAI::Difficulty> comPlayers;

static bool isComPlayer(int playerId);
static int comDelayCounter = 0;
static const int COM_DELAY = 50; // delay untuk COM
...
static SkillCard *pendingNewCard = nullptr;
static bool newPlayerTurnStarted = false;
...
static void checkMonopoly(Player *player, Board *board) {
  ...
  for (PropertyTile *prop : player->getOwnedProperties()) {
    if (prop->isStreet()) {
      ...
      static_cast<StreetTile *>(prop)->setMonopolized(ownedInGroup[cg] ==
                                                      totalInGroup[cg]);
    }
  }
}
...
static GameState buildGameState(GameEngine &engine, TransactionLogger &logger) {
  ...
  for (int i = 1; i <= board->getTotalTiles(); i++) {
    ...
    if (tile->isProperty()) {
      ...
      ti.festivalMult = prop->getFestivalMultiplier();
    }
    if (tile->isStreet()) {
  ...
  case Phase::AWAITING_ACTION:
    gs.currentPhase = "AWAITING_ACTION";
    break;
  case Phase::ROLLED:
    gs.currentPhase = "ROLLED";
    break;
  case Phase::EFFECT_PENDING:
    gs.currentPhase = "EFFECT_PENDING";
    break;
  case Phase::POST_ROLL:
    gs.currentPhase = "POST_ROLL";
    break;
  case Phase::AUCTION_ACTIVE:
    gs.currentPhase = "AUCTION_ACTIVE";
    break;
  case Phase::GAME_OVER:
    gs.currentPhase = "GAME_OVER";
    break;
  default:
    gs.currentPhase = "MENU";
    break;
  }
...
static void processTileEffect(GameEngine &engine, GameWindow &window,
  ...
  case EffectType::PAY_RENT: {
    ...
    if (p->isShieldActive()) {
      ...
      logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                      LogActionType::RENT,
                      "Sewa DICEGAH oleh Shield (" + prop->getKode() + ")");
      phase = Phase::POST_ROLL;
      break;
    }
    ...
    if (rent > 0) {
      try {
        ...
        *owner += rent;
        logger.logEvent(
            engine.getCurrentRound(), p->getUsername(), LogActionType::RENT,
            "Bayar " + fmtMoney(rent) + " ke " + owner->getUsername() + " (" +
                prop->getKode() + ")");
      } catch (NotEnoughMoneyException &) {
        int sisaUang = p->getMoney();
        *owner += sisaUang;
        auto bkProps = vector<PropertyTile *>(p->getOwnedProperties().begin(),
                                              p->getOwnedProperties().end());
    ...
    try {
      *p -= amount;
      ...
      for (PropertyTile *op : bkProps) {
        ...
        op->setStatus(0);
        if (op->isStreet())
          static_cast<StreetTile *>(op)->demolish();
        p->removeProperty(op);
      }
      p->declareBankruptcy();
      logger.logEvent(engine.getCurrentRound(), p->getUsername(),
  ...
  case EffectType::DRAW_COMMUNITY: {
    ...
    if (eff == ActionCardEffect::COMMUNITY_BIRTHDAY) {
      ...
      if (p->isShieldActive()) {
        ...
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::CARD,
                        "Dana Umum Dokter DICEGAH oleh Shield");
      } else {
        try {
          *p -= 700;
        } catch (NotEnoughMoneyException &) {
      ...
      if (p->isShieldActive()) {
        ...
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::CARD,
                        "Dana Umum Pemilu DICEGAH oleh Shield");
      } else {
        bool bankrupt = false;
        for (Player *other : allPlayers) {
          if (other != p && other->getStatus() != PlayerStatus::BANKRUPT) {
            try {
              ...
              *other += 200;
              ...
              auto bkProps =
                  vector<PropertyTile *>(p->getOwnedProperties().begin(),
                                         p->getOwnedProperties().end());
              for (PropertyTile *op : bkProps) {
                op->setOwnerId(-1);
                op->setStatus(0);
  ...
  case EffectType::FESTIVAL_TRIGGER: {
    ...
    for (auto *prop : props) {
      if (!prop->isStreet())
        continue; // Railroad/Utility butuh perubahan model
      int curMult = prop->getFestivalMultiplier();
      if (curMult >= 8)
        continue; // sudah maks
      string label = prop->getKode() + " - " + prop->getName();
      if (curMult > 1)
        label += " (saat ini x" + to_string(curMult) + " → x" +
                 to_string(curMult * 2) + ")";
      else
        label += " (x2, 3 giliran)";
    ...
    ps.title = "FESTIVAL";
    ps.message =
        "Pilih properti untuk efek festival (stackable x2→x4→x8, 3 giliran):";
    ps.options = opts;
    window.showPopup(ps);
  ...
  case EffectType::SEND_TO_JAIL: {
    // Shield block masuk penjara
    if (p->isShieldActive()) {
      p->setShieldActive(false);
      logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                      LogActionType::UNKNOWN,
                      "Masuk Penjara DICEGAH oleh Shield");
      phase = Phase::POST_ROLL;
      break;
    }
    p->goToJail();
  ...
  case EffectType::AWARD_SALARY: {
    ...
    *p += salary;
    ...
    // Dapat kartu skill pas lewat di GO
    ...
    if (drawn) {
      try {
        ...
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::CARD,
                        "Dapat kartu: " + drawn->getCardName());
      } catch (...) {
        // Tangan penuh (3 kartu), balikin ke deck
...
static void handlePopupResponse(int choice, GameEngine &engine,
    ...
    if (pendingTax) {
      ...
      try {
        *p -= amount;
        ...
        // Reset properti ke bank before bangkrut
        auto bkProps = vector<PropertyTile *>(p->getOwnedProperties().begin(),
                                              p->getOwnedProperties().end());
        for (PropertyTile *op : bkProps) {
          op->setOwnerId(-1);
  ...
  case Pending::FESTIVAL: {
    window.closePopup();
    auto props = p->getOwnedProperties();
    // Bangun ulang daftar yg sama pas popup dibuat (Street, maks <8)
    int selectIdx = 0;
    for (auto *prop : props) {
      if (!prop->isStreet())
        continue;
      ...
      if (selectIdx == choice) {
        ...
        st->setFestivalEffect(2);                // stacking
        festivalDurations[prop->getIndex()] = 3; // 3 round
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::FESTIVAL,
                        prop->getName() + " (x" +
                            to_string(st->getFestivalMultiplier()) + ")");
        break;
      }
      selectIdx++;
    }
    ...
    if (choice == 0) {
      try {
        *p -= fine;
        ...
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::UNKNOWN,
                        "Bayar denda penjara " + fmtMoney(fine));
      ...
      try {
        ...
        if (engine.getCurrentPlayer()->getStatus() == PlayerStatus::JAILED) {
          // Non double: tetap di penjara, - sisa percobaan
          p->decrementJailTurn();
          phase = Phase::POST_ROLL;
        } else {
          // Double: bebas dari penjara, proceed petak
          phase = Phase::ROLLED;
  ...
  case Pending::DROP_CARD: {
    ...
    if (choice >= 0 && choice < (int)p->getHandCards().size())
      p->removeCard(choice);
    // if drop dipicu oleh kartu awal giliran, masukkan kartu baru
    if (pendingNewCard) {
      try {
        p->addCard(pendingNewCard);
        logger.logEvent(
            engine.getCurrentRound(), p->getUsername(), LogActionType::CARD,
  ...
  case Pending::AKTA_SELECT: {
    ...
    if (choice >= 0 && choice < (int)aktaCandidates.size()) {
      ...
      if (prop->isStreet()) {
        lines.push_back("  [" + prop->getColorGroup() + "] " + prop->getName() +
                        " (" + prop->getKode() + ")");
        lines.push_back("+================================+");
  ...
  case Pending::TELEPORT_SELECT: {
    window.closePopup();
    if (choice >= 0 && choice < engine.getBoard()->getTotalTiles() &&
        pendingSkillCard) {
      // choice 0 = tile 1 = posisi 0
      int oldPos = p->getPosition();
      p->setPosition(choice);

      // Award gaji GO jika melewati GO (posisi baru < posisi lama, artinya
      // wrap)
      if (choice < oldPos) {
  ...
  case Pending::DEMOLITION_SELECT: {
    window.closePopup();
    if (choice >= 0 && choice < (int)demolitionCandidates.size() &&
        pendingSkillCard) {
    ...
    if (active.size() > 1) {
      ...
      for (Player *p : active) {
        if (p->getWealth() != top->getWealth() ||
            p->getOwnedProperties().size() !=
                top->getOwnedProperties().size() ||
...
static bool isComPlayer(int playerId) { return comPlayers.count(playerId) > 0; }

// Return difficulty COM (only valid if isComPlayer == true)
static ComputerAI::Difficulty getComDifficulty(int playerId) {
  auto it = comPlayers.find(playerId);
  if (it != comPlayers.end())
    return it->second;
  return ComputerAI::Difficulty::EASY;
}

// Initialize comPlayers based on nama player:
...
static void performRollDice(GameEngine &engine, GameWindow &window,
                            TransactionLogger &logger) {
  if (!gameStarted || phase != Phase::AWAITING_ACTION)
    return;
  ...
  logger.logEvent(engine.getCurrentRound(),
                  engine.getCurrentPlayer()->getUsername(), LogActionType::ROLL,
                  to_string(lastD1) + "+" + to_string(lastD2) + "=" +
                      to_string(lastD1 + lastD2));
...
static void performEndTurn(GameEngine &engine, GameWindow &window,
  ...
  // pemain ga BELI -> lelang
  Player *curP = engine.getCurrentPlayer();
  if (curP && pendingProp && pendingProp->getStatus() == 0 &&
      pendingProp->getOwnerId() == -1) {
    logger.logEvent(engine.getCurrentRound(), curP->getUsername(),
                    LogActionType::AUCTION,
                    "Lewat BELI → Lelang " + pendingProp->getName());
    startAuction(pendingProp, engine, window);
  ...
  // -- durasi festival di semua street (1 giliran per endTurn)
...
static void triggerComAction(GameEngine &engine, GameWindow &window,
  ...
  if (phase == Phase::AUCTION_ACTIVE && auctionCtx.active &&
    ...
    int choice = ComputerAI::decideAuction(
        bidder, auctionCtx.property, auctionCtx.highestBid, bid1, bid2, diff);
    handlePopupResponse(choice, engine, window, logger);
    return;
  }

  if (!p || !isComPlayer(p->getId()))
    return;
  ...
  case Phase::POST_ROLL:
    ...
    if (pendingProp && pendingProp->getStatus() == 0 &&
        pendingProp->getOwnerId() == -1) {
      int choice = ComputerAI::decideBuyProperty(p, pendingProp, diff);
      ...
      // choice != 0 atau tidak mampu: performEndTurn -> lelang
    }
    // HARD mode: coba bangun rumah/hotel sebelum mengakhiri giliran
    if (diff == ComputerAI::Difficulty::HARD) {
      int maxBuilds = 15; // batas loop buat cegah loop infinite
      while (maxBuilds-- > 0) {
        std::string target =
...
int main() {
  ...
  window.onLoadGame([&](string filename) {
    ...
    try {
      ...
      setupComPlayers(engine.getAllPlayers());
      comDelayCounter = 0;
      gameStarted = true;
      window.setScreen(AppScreen::PLAYING);
      phase = Phase::AWAITING_ACTION;
  ...
  window.onCommand("LEMPAR_DADU", [&]() {
    // Blokir if pemain saat ini COM
    Player *cp = engine.getCurrentPlayer();
    if (cp && isComPlayer(cp->getId()))
      return;
    performRollDice(engine, window, logger);
  });
    ...
    for (PropertyTile *prop : p->getOwnedProperties()) {
      if (prop->getStatus() == 1) {
        ...
        opts.push_back(prop->getKode() +
                       " - Gadai: " + fmtMoney(prop->getmortgageValue()));
      }
    }
    if (opts.empty())
      return;
  ...
  window.onCommand("BELI", [&]() {
    ...
    if (!tile || !tile->isProperty())
      return;
    PropertyTile *prop = static_cast<PropertyTile *>(tile);
    if (prop->getStatus() != 0 || prop->getOwnerId() != -1)
    ...
    if (p->getMoney() >= prop->getPrice()) {
      ...
      ps.message =
          "Kamu berada di " + prop->getName() + " (" + prop->getKode() + ")\n" +
  ...
  window.onCommand("CETAK_PAPAN", [&]() {
    ...
    for (int i = 1; i <= board->getTotalTiles(); i++) {
      ...
      string line = "[" + to_string(i) + "] " + tile->getKode() + " - " +
                    propStatusStr(pt->getStatus()) + " (" + ownerStr + ")";
      if (tile->isStreet()) {
        int rl = tile->getRentLevel();
        string bld = rl == 5 ? " | Hotel"
                             : (rl > 0 ? " | " + to_string(rl) + " Rumah" : "");
        line += bld;
        int fm = pt->getFestivalMultiplier();
        if (fm > 1)
          line += " | Festival x" + to_string(fm);
      }
      ...
      if (pl->getOwnedProperties().empty()) {
        ...
        for (PropertyTile *prop : pl->getOwnedProperties()) {
          ...
          if (prop->isStreet()) {
            ...
            info +=
                rl == 5 ? " Hotel" : (rl > 0 ? " " + to_string(rl) + "H" : "");
            int fm = prop->getFestivalMultiplier();
            if (fm > 1)
              info += " Fest x" + to_string(fm);
          }
  ...
  window.onCommand("CETAK_LOG", [&]() {
    ...
    vector<string> lines;
    lines.push_back("=== LOG TRANSAKSI (" + to_string(logs.size()) +
                    " entri) ===");
    int from = (int)logs.size() > 20 ? (int)logs.size() - 20 : 0;
    for (int i = from; i < (int)logs.size(); i++) {
      auto &le = logs[i];
      lines.push_back("[T" + to_string(le.turn) + "] [" +
                      actionTypeToString(le.actionType) + "] " + le.username +
                      ": " + le.detail);
    }
    if ((int)logs.size() > 20)
      lines.push_back("... (hanya 20 terakhir ditampilkan)");
  ...
  window.onCommand("SIMPAN", [&]() {
    if (!gameStarted)
      return;
    window.showTextInput("SIMPAN PERMAINAN",
                         "Path file (contoh: saves/game1):", true);
  });

  window.onCommand("MUAT", [&]() {
    window.showTextInput("MUAT PERMAINAN",
                         "Path file (contoh: saves/game1):", false);
  });
  for (int ci = 0; ci < 3; ci++) {
    window.onCommand("GUNAKAN_KEMAMPUAN_" + to_string(ci), [&, ci]() {
      ...
      case SkillCardEffect::MOVE: {
        ...
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::CARD,
                        "MoveCard maju " + to_string(steps) + " petak");
        engine.markDiceRolled();
        phase = Phase::ROLLED;
        processTileEffect(engine, window, logger);
        break;
      ...
      case SkillCardEffect::DISCOUNT: {
        ...
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::CARD,
                        "DiscountCard diskon " + to_string(pct) + "%");
        removeUsedCard(ci);
        break;
      }
      case SkillCardEffect::SHIELD: {
        ...
        logger.logEvent(engine.getCurrentRound(), p->getUsername(),
                        LogActionType::CARD,
                        "ShieldCard: imun tagihan giliran ini");
        removeUsedCard(ci);
        break;
      }
      case SkillCardEffect::TELEPORT: {
        // Popup pilih tujuan (semua petak 1–40)
      ...
      case SkillCardEffect::DEMOLITION: {
        ...
        for (Player *pl : engine.getAllPlayers()) {
          ...
          for (PropertyTile *prop : pl->getOwnedProperties()) {
            ...
            if (prop->getStatus() == 2)
              lvl = "[TERGADAI]";
            else if (st->getRentLevel() == 0)
              lvl = "Tanah kosong";
            else if (st->getRentLevel() == 5)
              lvl = "Hotel";
            else
              lvl = to_string(st->getRentLevel()) + " Rumah";
            opts.push_back(st->getKode() + " [" + pl->getUsername() + "] " +
                           lvl);
          }
        }
        if (demolitionCandidates.empty())
          break;
        pendingSkillCard = card;
        PopupState ps;
        ps.type = PopupType::BUY_PROPERTY;
  ...
  while (window.isRunning()) {
    ...
    if (window.getScreen() == AppScreen::PLAYING && gameStarted) {
      if (turnJustStarted && phase == Phase::AWAITING_ACTION) {
        ...
        // Jail popup, hold if sedang DROP_CARD
      ...
      if (phase != Phase::GAME_OVER) {
        ...
        if (phase == Phase::AUCTION_ACTIVE && auctionCtx.active &&
            !auctionCtx.bidders.empty()) {
          // Cek if penawar lelang saat ini adalah COM
          needComAction =
              isComPlayer(auctionCtx.bidders[auctionCtx.currentIdx]->getId());
          ...
          if (isComPlayer(cp->getId())) {
            ...
            case Phase::AWAITING_ACTION:
              ...
              break;
            case Phase::EFFECT_PENDING:
              needComAction = (pending == Pending::BUY_PROPERTY ||
                               pending == Pending::TAX_CHOICE ||
                               pending == Pending::FESTIVAL ||
                               pending == Pending::JAIL_CHOICE ||
                               pending == Pending::DROP_CARD);

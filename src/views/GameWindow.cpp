...
void GameWindow::onExitGame(std::function<void()> cb) { exitGameCallback = cb; }
...
void GameWindow::drawMainMenu() {
  ...
  drawPixelText(title1, (int)(cx + (cw - tw1) / 2), (int)cy + 18, 26,
                C_BTN_TEXT);
  ...
  drawPixelText(title2, (int)(cx + (cw - tw2) / 2), (int)cy + 58, 10,
                {216, 200, 154, 255});
  drawPixelText("v1.0", (int)(cx + cw - 48), (int)cy + 10, 8,
                {122, 104, 64, 255});
...
void GameWindow::drawPlayerSetup() {
  ...
  // Hitung nama efektif: bot mendapat nama otomatis "COM"/"COM2"/...
  // "GOD"/"GOD2"/...
  auto effectiveNames = [&]() {
    std::vector<std::string> result;
    int easyCount = 0, hardCount = 0;
    for (int i = 0; i < menuState.numPlayers; i++) {
      int t = menuState.playerTypes[i];
      if (t == 1) {
        easyCount++;
        result.push_back(easyCount == 1 ? "COM"
                                        : "COM" + std::to_string(easyCount));
      } else if (t == 2) {
        hardCount++;
        result.push_back(hardCount == 1 ? "GOD"
                                        : "GOD" + std::to_string(hardCount));
      } else {
        result.push_back(menuState.playerNames[i]);
      }
    }
    return result;
  };
  ...
  for (int i = 0; i < menuState.numPlayers; i++) {
    ...
    int ptype = menuState.playerTypes[i];
    bool isHuman = (ptype == 0);
    bool isActive = isHuman && (menuState.activeField == i);
    ...
    // Type toggle button (kanan)
    float toggleW = 110.0f, toggleH = 34.0f;
    Rectangle toggleRect = {cx + cw - 46 - toggleW, fy + 22, toggleW, toggleH};
    static const Color typeBg[3] = {
        {60, 140, 60, 255}, {50, 100, 200, 255}, {180, 50, 30, 255}};
    static const char *typeLabel[3] = {"PLAYER", "BOT EASY", "BOT HARD"};
    bool thov = isHovered(toggleRect);
    DrawRectangleRec({toggleRect.x + 3, toggleRect.y + 3, toggleRect.width,
                      toggleRect.height},
                     {0, 0, 0, 80});
    DrawRectangleRec(toggleRect,
                     thov ? tint(typeBg[ptype], 20) : typeBg[ptype]);
    DrawRectangleLinesEx(toggleRect, 2, C_BORDER);
    int tlw = MeasureText(typeLabel[ptype], 8);
    drawPixelText(typeLabel[ptype],
                  (int)(toggleRect.x + (toggleRect.width - tlw) / 2),
                  (int)(toggleRect.y + (toggleRect.height - 8) / 2), 8, WHITE);
    if (thov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
      menuState.playerTypes[i] = (ptype + 1) % 3;
      if (menuState.activeField == i)
    ...
    if (isHuman) {
      ...
      std::string shown = menuState.playerNames[i] + (showCursor ? "|" : "");
      if (menuState.playerNames[i].empty() && !showCursor)
        ...
        drawPixelText(shown, (int)inputRect.x + 10, (int)inputRect.y + 11, 10,
                      C_TEXT);
      if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && isHovered(inputRect))
        menuState.activeField = i;
    } else {
      // Bot: tampilkan nama otomatis di kotak abu
      DrawRectangleRec(inputRect, Color{210, 210, 205, 255});
      DrawRectangleLinesEx(inputRect, 1, C_BORDER);
      auto enames = effectiveNames();
      drawPixelText(enames[i], (int)inputRect.x + 10, (int)inputRect.y + 11, 10,
                    C_TEXT_DIM);
    }
  }
  ...
  if (menuState.activeField >= 0 &&
      ...
      menuState.playerTypes[menuState.activeField] == 0) {
    handleKeyboardTextInput(menuState.playerNames[menuState.activeField], 16);
    if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_TAB)) {
      // advance to next human slot
      int next = (menuState.activeField + 1) % menuState.numPlayers;
      menuState.activeField = next;
    }
  }

  // Validasi: semua slot manusia harus terisi
  bool allFilled = true;
  for (int i = 0; i < menuState.numPlayers; i++)
    if (menuState.playerTypes[i] == 0 && menuState.playerNames[i].empty()) {
      allFilled = false;
      break;
    }
  ...
  for (int i = 0; i < menuState.numPlayers && !hasDuplicate; i++)
    for (int j = i + 1; j < menuState.numPlayers && !hasDuplicate; j++)
      if (!enames[i].empty() && enames[i] == enames[j])
        hasDuplicate = true;

  bool allBot = true;
  for (int i = 0; i < menuState.numPlayers; i++)
    if (menuState.playerTypes[i] == 0) {
      allBot = false;
      break;
    }
  ...
  if (!allFilled) {
    ...
    drawPixelText(
        "Peringatan: semua pemain adalah bot! (lanjut jika anda yakin!)",
        (int)(cx + 30), (int)(cy + ch - 96), 12, C_DANGER);
  }

  float sbY = cy + ch - 76;
  float sbW = (cw - 100) * 0.65f;
  Rectangle startBtn = {cx + 40, sbY, sbW, 44};
  bool canStart = allFilled && !hasDuplicate;
  bool shov = isHovered(startBtn) && canStart;
  Color sbCol = canStart ? (shov ? tint(C_ACCENT, 20) : C_ACCENT)
                         : Color{160, 160, 155, 255};
  DrawRectangleRec(
      {startBtn.x + 4, startBtn.y + 4, startBtn.width, startBtn.height},
      {0, 0, 0, 80});
  ...
  if (canStart && shov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
    if (newGameCallback)
      newGameCallback(menuState.numPlayers, effectiveNames());
  }
  ...
  DrawRectangleRec(
      {backBtn.x + 4, backBtn.y + 4, backBtn.width, backBtn.height},
      {0, 0, 0, 80});
  ...
  if (bhov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
    ...
    for (auto &n : menuState.playerNames)
      n = "";
    menuState.activeField = 0;
  }
}
...
void GameWindow::drawTextInputOverlay() {
  ...
  drawPixelText(textInput.title, (int)(px + (pw - htw) / 2), (int)py + 17, 13,
                WHITE);
  ...
  if (textInput.buffer.empty() && !cursor) {
    ...
    drawPixelText(shown, (int)inputRect.x + 10, (int)inputRect.y + 12, 10,
                  C_TEXT);
  }
  ...
  if (IsKeyPressed(KEY_ENTER) && !textInput.buffer.empty()) {
    confirm();
    return;
  }
  ...
  if (cahov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
    cancel();
    return;
  }
}
...
void GameWindow::drawTile(const TileInfo &tile, Rectangle rect) {
  ...
  // Festival multiplier badge
  if (tile.festivalMult > 1) {
    std::string flab = "x" + std::to_string(tile.festivalMult);
    int flw = MeasureText(flab.c_str(), 6) + 4;
    int fmx, fmy;
    if (isSideBtm) {
      fmx = (int)rect.x + 2;
      fmy = (int)rect.y + 22;
      ...
      fmx = (int)rect.x + 2;
      fmy = (int)rect.y + (int)rect.height - 32;
    } else if (isSideLft) {
      fmx = (int)rect.x + (int)rect.width - flw - 2;
      fmy = (int)rect.y + 22;
    } else {
      fmx = (int)rect.x + 2;
      fmy = (int)rect.y + 22;
    }
    DrawRectangle(fmx, fmy, flw, 9, {231, 135, 28, 220});
    DrawRectangleLines(fmx, fmy, flw, 9, C_BORDER);
    drawPixelText(flab, fmx + 2, fmy + 1, 6, WHITE);
  }
...
void GameWindow::drawPlayerPawns() {
  ...
  for (auto &[pos, indices] : posMap) {
    ...
    // Sesuaikan ukuran pion agar muat di tile kecil
    int pionSz = (r.width < 40 || r.height < 40) ? 12 : 18;
    int step = pionSz + 2;
    for (int j = 0; j < n; j++) {
      Color pc = getPlayerColor(indices[j]);
      // Tata grid 2 kolom, clamp agar tidak keluar batas tile
      float ox = (float)((j % 2) * step);
      float oy = (float)((j / 2) * step);
      ox = std::min(ox, r.width - (float)pionSz - 2);
      oy = std::min(oy, r.height - (float)pionSz - 2);
      int px = (int)(r.x + r.width - (float)pionSz - 2 - ox);
      int py = (int)(r.y + r.height - (float)pionSz - 2 - oy);
      drawPion(gAssets, px, py, pionSz, pc);
    }
  }
}
...
void GameWindow::drawPlayerList() {
  ...
  for (int i = 0; i < maxRows; i++) {
    ...
    drawPixelText("@" + std::to_string(p.position), sx + sw - 42, ry + 13, 8,
                  C_TEXT_DIM);
    if (p.status == "JAILED") {
      int bx = sx + 160, by2 = ry + 9;
      std::string jlbl = "JAIL(" + std::to_string(p.jailTurnsLeft) + ")";
      int jlw = MeasureText(jlbl.c_str(), 7) + 10;
      DrawRectangle(bx, by2, jlw, 18, C_DANGER);
      DrawRectangleLines(bx, by2, jlw, 18, C_BORDER);
      drawPixelText(jlbl, bx + 5, by2 + 5, 7, WHITE);
    } else if (p.status == "BANKRUPT") {
      int bx = sx + 150, by2 = ry + 9;
      DrawRectangle(bx, by2, 74, 18, {90, 90, 90, 255});
    ...
    if (p.shieldActive) {
      ...
      drawPixelText("SHD", sbx + 2, sby + 1, 6, WHITE);
    }
    // COM badge
    if (p.isCom) {
      int cbx = sx + 40, cby = ry + 24;
      drawPixelText("BOT", cbx, cby, 6, C_WARN);
    }
  }
}
...
void GameWindow::drawHandCards() {
  ...
  for (int i = 0; i < (int)active.handCards.size() && i < 3; i++) {
    ...
    Rectangle ur = {(float)(sx + sw - 24 - 72), (float)(ry + 6), 66.0f,
                    (float)(cardH - 12)};
    bool hov = isHovered(ur) && cardUsable;
    Color pakBg = cardUsable ? C_BTN_BG : Color{150, 148, 140, 255};
    Color pakFg = cardUsable ? C_BTN_TEXT : Color{200, 196, 185, 255};
    drawButton("PAKAI", ur, pakBg, pakFg, hov);
    if (hov && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
...
void GameWindow::drawLog() {
  ...
  for (int i = 0; i < show; i++) {
    ...
    std::string logLine =
        "[" + log.actionType + "] " + log.username + ": " + log.detail;
    drawPixelText(truncate(logLine, 34), sx + 50, ry + 7, 8,
                  lat ? C_TEXT : C_TEXT_DIM);
  }
}
...
void GameWindow::drawSidebar() {
  ...
  // Label COM / shield di banner giliran
  std::string gilLabel = "GILIRAN: " + activeName;
  bool curIsShield = !state.players.empty() &&
                     state.players[state.currentPlayerIndex].shieldActive;
  bool curIsCom =
      !state.players.empty() && state.players[state.currentPlayerIndex].isCom;
  drawPixelText(gilLabel, sx + 40, by + 8, 10, {29, 93, 39, 255});
  if (curIsShield) {
    drawPixelText("[SHIELD]", sx + 40, by + 23, 7, {38, 120, 200, 255});
  }
  if (curIsCom) {
    // Indikator COM dengan animasi sederhana (teks berkedip)
  ...
  if (state.isComTurn && !curIsShield) {
    ...
    std::string think = "Berpikir" + std::string(dot1 ? "." : "") +
                        std::string(dot2 ? "." : "");
    drawPixelText(think, sx + 40, by + 23, 7, C_TEXT_DIM);
  }
...

  {
    ...
    Rectangle er = {(float)(sx + 14) + 2 * (bw3 + gap3), (float)saveY, bw3,
                    30.0f};
...
void GameWindow::drawCommandBar() {
  ...
  auto isEnabled = [&](const std::string &cmd) -> bool {
    ...
    if (cmd == "AKHIRI_GILIRAN")
      return (postRoll || awaiting) && state.dice.hasRolled && !comTurn;
    if (cmd == "BELI")
      return postRoll && !comTurn && state.canBuyCurrentTile;
    if (cmd == "GADAI")
      return canAct;
    if (cmd == "TEBUS")
      return canAct;
    if (cmd == "BANGUN")
      return canAct;
    // Info commands always available
    return !comTurn;
  };
  ...
  for (int i = 0; i < nb; i++) {
    ...
    bool enabled = isEnabled(b.cmd);
    bool hov = isHovered(bRect) && enabled;

    // Warna disabled: abu-abu
    Color bgCol = enabled ? b.bg : Color{160, 158, 150, 255};
    Color fgCol = enabled ? b.fg : Color{110, 105, 95, 255};

    DrawRectangleRec({bRect.x + 3, bRect.y + 3, bRect.width, bRect.height},
                     C_SHADOW);
    DrawRectangleRec(bRect, hov ? tint(bgCol, 15) : bgCol);
    DrawRectangleLinesEx(bRect, 2,
                         enabled ? C_BORDER : Color{130, 128, 120, 255});
    ...
    if (iconTex) {
      ...
      Rectangle iconRect = {bRect.x + (bRect.width - iconSz) / 2.0f, iconY,
                            iconSz, iconSz};
      drawIconInRect(iconTex, iconRect, fgCol);
    }
...
void GameWindow::drawPopup() {
  ...
  if (!useList) {
    ...
    for (int i = 0; i < (int)popup.options.size(); i++) {
      ...
      DrawRectangleRec(ir, hov ? C_ACCENT_BG
                               : (i % 2 == 0 ? C_PANEL : C_PANEL_ALT));

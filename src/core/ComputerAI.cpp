...
int ComputerAI::decideBuyProperty(const Player *player,
                                  const PropertyTile *prop, Difficulty diff) {
  if (!player || !prop)
    return 1;

  if (diff == Difficulty::HARD) {
    // Selalu beli if mampu
...
int ComputerAI::decideJailChoice(const Player *player, int fine,
                                 Difficulty diff) {
  if (!player)
    return 1;

  if (diff == Difficulty::HARD) {
    // Bayar denda if punya cukup uang
...
int ComputerAI::decideAuction(const Player *bidder, const PropertyTile *prop,
  ...
  if (diff == Difficulty::HARD) {
    ...
    if (bid1 > maxBid || bid1 > bidder->getMoney())
      return 2; // pass
    if (bid2 <= maxBid && bid2 <= bidder->getMoney())
      return 1; // tawar tinggi
    return 0;   // tawar rendah
  } else {
    // EASY: 30% kemungkinan tawar minimum, sisanya pass
    std::uniform_int_distribution<> d(0, 9);
    if (d(getRng()) < 3 && bid1 <= bidder->getMoney())
      return 0;
    return 2;
  }
}
...
std::string ComputerAI::decideBuildTarget(const Player *player, Board *board,
                                          Difficulty diff) {
  if (!player || !board || diff == Difficulty::EASY)
    return "";

  // Count total petak per color group di board
  ...
  for (const auto &[color, streets] : ownedInGroup) {
    ...
    if (minLevel == INT_MAX || minLevel >= 5)
      continue; // semua sudah hotel

    // Pilih properti dengan level terendah yang mampu dibayar
    for (const StreetTile *st : streets) {
      if (st->getStatus() != 1)
        continue;
      if (st->getRentLevel() != minLevel)
        continue;
      if (st->getRentLevel() >= 5)
        continue;

      int cost =
          (st->getRentLevel() == 4) ? st->getHotelCost() : st->getHouseCost();
      if (player->getMoney() > cost + 200)
        return st->getKode();
    }
  }
  return "";

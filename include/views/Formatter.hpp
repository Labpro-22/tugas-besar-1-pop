...
public:
    ...
    static void printBayarSewa(const Player& payer, const Player& owner, const PropertyTile& property, int rentAmount);
    static void printBayarPajak(const Player& player, int amount);
    static void printAuction(const PropertyTile& property, const Player& highestBidder, int highestBid);
    static void printFestival(const PropertyTile& property, int oldRent, int newRent, int duration);
    static void printHandCards(const Player& player);
    static void printWinner(const std::vector<Player>& players, bool isBankruptcy);
    static void printBankrupt(const Player& player, const std::string& creditor);
};
